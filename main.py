...
tasks = get_tasks()
if tasks:
    counter = 1  # Initialize counter for task numbers
    for task in tasks:
        task_id = task['ID']
        with st.container():
            ...
            cols = st.columns(3)
            with cols[0]:
                task_description = st.text_input(f"Description_{counter}", value=task['Description'])
            with cols[1]:
                task_status = st.selectbox(f"Status_{counter}", ["Pending", "Completed"], index=["Pending", "Completed"].index(task['Status']))
            with cols[2]:
                update_button = st.button(f"Update", key=f"update_{task_id}", use_container_width=True)
                delete_button = st.button(f"Delete", key=f"delete_{task_id}", use_container_width=True)
            ...
            if update_button:
                if update_task(task_id, task_description, task_status):
                    st.success(f"Task {counter} updated successfully!")
                else:
                    st.error(f"Failed to update task {counter}!")
            if delete_button:
                if delete_task(task_id):
                    st.success(f"Task {counter} deleted successfully!")
                else:
                    st.error(f"Failed to delete task {counter}!")
        
        counter += 1  # Increment counter after processing each task
else:
    st.warning("No tasks available.")
